...
创建完成后记录 ELB 名称和 IPv4 私有地址。这里先不手工添加 Pod IP；下一步由 CCE Service 绑定这个 ELB，并管理监听器和后端。

### 第二步：创建 CCE 负载均衡 Service

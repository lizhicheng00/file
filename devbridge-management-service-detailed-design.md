...
| 默认 API Key | 用户按类型首次进入时幂等创建，重复获取结果稳定 |
| 类型 API Key | 用户可以为 DevBridge、DevBox 或不同设备创建独立 Key |
| 身份解析 | 校验 API Key，并返回用户身份、用户 Namespace 和账户 Namespace |
...
| 项目 | 规则 |
| --- | --- |
| 默认 Key | `devbridge`、`devbox` 各 1 个，按类型获取，可重复获取，不可删除 |
...
- 各类型默认 Key 用于首次进入和持续管理，附加 Key 用于设备隔离；
...
| 流程 | 触发 | 核心处理 | 结果 |
| --- | --- | --- | --- |
| 获取默认 Key | 用户完成登录并选择类型 | 根据 Domain ID、User ID 和类型创建或读取身份、Namespace 和默认 Key | 返回该类型稳定的默认 Key |
| 校验身份 | 上层服务收到 API Key | 校验 Key，并找到用户及账户归属 | 返回两个 Namespace，或认证失败 |
...
- DevBridge 和 DevBox 分别获取自己的默认 Key，也可以用已有有效 Key 持续管理其他 Key；
...
默认 Key 根据用户身份和类型稳定生成。同一类型重复调用不会增加 Key 数量，也不会改变 Namespace；不同类型得到不同的默认 Key。
...
服务根据 API Key 摘要找到凭证记录，再关联用户和账户。摘要用于快速、唯一地定位记录，不能还原完整 Key。成功解析同时更新最近使用时间；`GET /me` 即为 Key 校验和身份解析接口。
...
| 方法与路径 | 身份信息 | 用途 |
| --- | --- | --- |
| `POST /api-key` | 上层服务凭证、Domain ID、User ID、类型 | 幂等创建或获取该类型默认 Key |
| `GET /me` | API Key | 校验 Key，解析当前用户及 Namespace |
...
| 业务范围 | 建议场景 | 预期结果 |
| --- | --- | --- |
| 身份映射 | 使用相同 Domain ID、User ID 和类型重复获取默认 Key | 用户 Namespace、账户 Namespace 和默认 Key 保持不变 |
| 账户共享 | 同一 Domain 下使用两个不同 User ID | 用户 Namespace 不同，账户 Namespace 相同 |
| 账户隔离 | 使用两个不同 Domain ID | 账户 Namespace 和用户 Namespace 均不同 |
| 默认 Key | 分别获取两个类型的默认 Key 并重复调用 | 每个类型只有一个稳定默认 Key，两个 Key 值不同 |
...
| 最近使用 | 使用一个 Key 调用 `/me` 后查询列表 | 对应 Key 的 `lastUsedAt` 已记录 |
